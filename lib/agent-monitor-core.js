...
export function buildEventText(type, { title, projectName, deadline }) {
  ...
  if (type === "overdue") {
    return `⚠️ Задача просрочена: «${title}»${proj}. Срок был ${deadline}.`;
  }
  if (type === "deadline_tomorrow") {
    return `⏰ Остался 1 день: «${title}»${proj}. Срок — ${deadline}.`;
  }
  if (type === "not_taken_1h") {
    return `❗️ Задача больше часа не взята в работу: «${title}»${proj}.`;
  }
  return `🔔 «${title}»${proj}`;
}
