...
function telegramFactLines({ title, projectName, deadline, assigneeNames }, deadlineLabel = "Срок") {
  const names = (Array.isArray(assigneeNames) ? assigneeNames : [assigneeNames])
    .map((name) => String(name || "").trim())
    .filter(Boolean);
  return [
    `<b>Задача:</b> ${escapeTelegramHtml(title || "Без названия")}`,
    projectName ? `<b>Проект:</b> ${escapeTelegramHtml(projectName)}` : null,
    names.length > 0
      ? `<b>${names.length > 1 ? "Ответственные" : "Ответственный"}:</b> ${escapeTelegramHtml(names.join(", "))}`
      : null,
    deadline ? `<b>${deadlineLabel}:</b> ${escapeTelegramHtml(formatIsoDayRu(deadline))}` : null,
  ].filter(Boolean);
}

// Telegram gets a compact card-like layout. The in-app feed/push keep using
// buildEventText above because they must remain plain text.
export function buildTelegramEventText(type, data) {
  const facts = telegramFactLines(data, type === "overdue" ? "Срок был" : "Срок");
  if (type === "overdue") {
    const age = daysBetween(data.deadline, data.today);
    if (age !== null && age > 0) {
      facts.push(`<b>Просрочка:</b> ${age} ${pluralRu(age, "день", "дня", "дней")}`);
    }
    return [`⚠️ <b>Задача просрочена</b>`, "", ...facts].join("\n");
  }
  if (type === "deadline_today") {
    return [`⏰ <b>Срок по задаче сегодня</b>`, "", ...facts].join("\n");
  }
  if (type === "deadline_tomorrow") {
    return [`⏳ <b>До срока остался 1 день</b>`, "", ...facts].join("\n");
  }
  if (type === "not_taken_1h") {
    return [`❗️ <b>Задача не взята в работу</b>`, "", ...facts.filter(line => !line.startsWith("<b>Срок:"))].join("\n");
  }
  if (type === "unassigned_1h") {
    return [`👤 <b>У задачи нет ответственного</b>`, "", ...facts.filter(line => !line.startsWith("<b>Ответственн") && !line.startsWith("<b>Срок:"))].join("\n");
  }
  return [`🔔 <b>Уведомление по задаче</b>`, "", ...facts].join("\n");
}

export function appendTelegramAdvice(baseText, advice) {
  const base = String(baseText || "").trim();
  const cleanAdvice = String(advice || "").trim();
  if (!cleanAdvice) return base;
  return `${base}\n\n💡 <b>Рекомендация:</b> ${escapeTelegramHtml(cleanAdvice)}`;
}
...
export function buildTelegramDigestText(type, { count, titles }) {
  const n = Number(count) || 0;
  const noun = pluralRu(n, "задача", "задачи", "задач");
  const label = (DIGEST_TYPE_LABELS[type] || (() => "требуют внимания"))(n);
  const shown = (Array.isArray(titles) ? titles : [])
    ...
    .slice(0, 3);
  const lines = [
    `📋 <b>Сводка ProjectSfera</b>`,
    "",
    `<b>${n} ${noun}</b> ${escapeTelegramHtml(label)}.`,
  ];
  if (shown.length > 0) {
    lines.push("", ...shown.map(title => `• ${escapeTelegramHtml(title)}`));
    if (n > shown.length) lines.push(`• И ещё ${n - shown.length}`);
  }
  return lines.join("\n");
}
