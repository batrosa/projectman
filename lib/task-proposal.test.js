...
describe("matchAssignee — email, логин-часть и тёзки", () => {
  const twins = [
    ...
    { id: "u-other", displayName: "Тэко Исаев", email: "teko@example.com" },
  ];

  it("полное имя тёзок → ambiguous со списком кандидатов (имя + email)", () => {
    const result = matchAssignee(twins, "Вячеслав Гурьев");
    expect(result.error).toBe("ambiguous");
    expect(result.options).toHaveLength(2);
    expect(result.options.map((o) => o.email).sort()).toEqual(["guryev.cio@gmail.com", "stargyrev@gmail.com"]);
  });

  it("точный email и начало логина различают тёзок", () => {
    expect(matchAssignee(twins, "guryev.cio@gmail.com").uid).toBe("u-cio");
    expect(matchAssignee(twins, "guryev").uid).toBe("u-cio");
    expect(matchAssignee(twins, "stargyrev").uid).toBe("u-star");
  });

  it("слишком короткий или общий фрагмент не матчится по email", () => {
    expect(matchAssignee(twins, "gur").error).toBeTruthy();
    expect(matchAssignee(twins, "gmail").error).toBeTruthy();
  });
});
...
describe("matchAssignee", () => {
  ...
  it("ambiguity → error 'ambiguous' (никогда не угадываем между двумя людьми)", () => {
    ...
    expect(matchAssignee(dup, "Иван Петров")).toMatchObject({ error: "ambiguous" });
  });
