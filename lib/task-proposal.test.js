...
import { extractProposal, validateProposal, matchAssignee, validateCreateTasksPayload } from "./task-proposal.js";
...
describe("validateProposal", () => {
  ...
  it("trims strings and rejects empty/oversized titles", () => {
    expect(validateProposal({ ...good, tasks: [{ title: "   ", deadline: null, assigneeName: "X" }] }).ok).toBe(false);
    expect(validateProposal({ ...good, tasks: [{ title: "x".repeat(201), deadline: null, assigneeName: "X" }] }).ok).toBe(false);
  });

  it("rejects a bad deadline format and impossible dates", () => {
    expect(validateProposal({ ...good, tasks: [{ title: "T", deadline: "10.07.2026", assigneeName: "X" }] }).ok).toBe(false);
    expect(validateProposal({ ...good, tasks: [{ title: "T", deadline: "2026-13-45", assigneeName: "X" }] }).ok).toBe(false);
  });

  it("rejects: no file, empty tasks, >30 tasks, missing assigneeName", () => {
    expect(validateProposal({ ...good, file: "" }).ok).toBe(false);
    expect(validateProposal({ ...good, tasks: [] }).ok).toBe(false);
    expect(validateProposal({ ...good, tasks: Array.from({ length: 31 }, (_, i) => ({ title: `T${i}`, deadline: null, assigneeName: "X" })) }).ok).toBe(false);
    expect(validateProposal({ ...good, tasks: [{ title: "T", deadline: null, assigneeName: "" }] }).ok).toBe(false);
