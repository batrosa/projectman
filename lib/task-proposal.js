...
export function matchAssignee(users, name) {
  ...
  // Email или его логин-часть: «guryev.cio@gmail.com», «guryev.cio», «guryev».
  // Полный email — точное совпадение; фрагмент (≥4 символов, латиница) —
  // единственное вхождение в логин-часть email среди участников. Нужен для
  // уточнения исполнителя, когда в организации два полных тёзки и различить
  // их можно только по email (прод-кейс: два «Вячеслав Гурьев»).
  const emailCandidate = String(name || "").trim().toLowerCase();
  if (/^[a-z0-9._%+-]+@[a-z0-9.-]+$/.test(emailCandidate)) {
    const emailHits = users.filter((user) => user?.id
      && String(user.email || "").toLowerCase() === emailCandidate);
    if (emailHits.length === 1) return matchedUserResult(emailHits[0]);
    if (emailHits.length > 1) return ambiguousResult(emailHits);
  } else if (/^[a-z0-9._-]{4,}$/.test(emailCandidate)) {
    const localHits = users.filter((user) => {
      if (!user?.id) return false;
      const local = String(user.email || "").toLowerCase().split("@")[0];
      return local && (local === emailCandidate || local.startsWith(emailCandidate));
    });
    if (localHits.length === 1) return matchedUserResult(localHits[0]);
  }
  ...
  return matchedUserResult(hits[0]);
}

function matchedUserResult(user) {
...
// «ambiguous» с перечнем кандидатов: вызывающий может честно спросить
// пользователя, КОГО из тёзок он имел в виду (имя + email для различения).
function ambiguousResult(hits) {
  return {
    error: "ambiguous",
    options: hits.slice(0, 6).map((user) => ({
      uid: user.id,
