...
import { canCallerReadPrivateTask, loadTaskDocument, PRIVATE_TASKS_COLLECTION } from "./task-store.js";
import {
  buildPublicId,
...
async function handleDeleteTask({ response, db, decoded, body }) {
  ...
  const loaded = await loadTaskDocument(db, taskId, body.taskCollection);
  if (!loaded) return response.status(404).json({ error: "Task not found" });
  const { ref: taskRef, task } = loaded;
  const access = await loadAccess(db, decoded.uid, task.projectId);
  // Удалять может менеджер проекта, СОЗДАТЕЛЬ задачи (исполнитель — свои
  // задачи) или доп. постановщик этой задачи.
  const callerIsTaskCreator = task.createdByUid === decoded.uid
    || (Array.isArray(task.coCreatorIds) && task.coCreatorIds.includes(decoded.uid));
  if (!access || (!access.canManage && !callerIsTaskCreator)) {
    return response.status(403).json({ error: "Forbidden" });
  }
  if (loaded.collectionName === PRIVATE_TASKS_COLLECTION
      && !canCallerReadPrivateTask(task, { uid: decoded.uid, ...access })) {
    return response.status(403).json({ error: "Forbidden" });
  }
