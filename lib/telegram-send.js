...
export function buildProjectSferaLink({ taskId, projectId, organizationId, taskCollection } = {}) {
  ...
  if (taskCollection === "privateTasks") url.searchParams.set("taskCollection", "privateTasks");
  return url.toString();
}

function projectManReplyMarkup({ taskId, projectId, organizationId, taskCollection, linkToProjectSfera } = {}) {
  if (!linkToProjectSfera && !taskId) return null;
  const hasTask = ID_RE.test(String(taskId || ""));
  return {
    inline_keyboard: [[{
      text: hasTask ? "📋 Открыть задачу" : "🚀 Открыть ProjectSfera",
      url: buildProjectSferaLink({ taskId, projectId, organizationId, taskCollection }),
    }]],
  };
}
