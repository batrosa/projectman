...
import { loadTaskDocumentForCaller } from "./task-store.js";
...
async function deliver(user, { title, plainText, htmlText, data }) {
  ...
  await Promise.allSettled([
    ...
    user.telegramChatId
      ? sendTelegramMessage(String(user.telegramChatId), htmlText, {
          parseMode: "HTML",
          taskId: data?.taskId,
...
async function requestDeadlineChange({ db, decoded, caller, organizationId, body }) {
  ...
  if (!loaded) return { status: 404, error: "Задача не найдена" };
  if (loaded.forbidden) return { status: 403, error: "Нет доступа к задаче" };
  const { ref: taskRef, task, collectionName: taskCollection } = loaded;
  ...
  await Promise.allSettled(recipients.map((recipient) => deliver(recipient, {
    ...
    data: { type: "deadline_change_requested", taskId, projectId, taskCollection, requestId: requestRef.id },
  })));
...
async function decideDeadlineChange({ db, decoded, caller, organizationId, body }) {
  ...
  const taskCollection = requestData.taskCollection === "privateTasks" ? "privateTasks" : "tasks";
  const loadedTask = await loadTaskDocumentForCaller(db, taskId, taskCollection, {
    uid: decoded.uid,
    organizationId,
    orgRole: caller?.orgRole,
  });
  if (!loadedTask) return { status: 404, error: "Задача не найдена" };
  if (loadedTask.forbidden) return { status: 403, error: "Нет доступа к задаче" };
  const taskRef = loadedTask.ref;
  ...
  if (requesterSnap?.exists && requesterSnap.data().organizationId === organizationId) {
    ...
    await deliver(requester, {
      ...
      data: { type: approved ? "deadline_change_approved" : "deadline_change_rejected", taskId, projectId: requestData.projectId, taskCollection, requestId },
    });
  }
