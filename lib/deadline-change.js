...
async function decideDeadlineChange({ db, decoded, caller, organizationId, body }) {
  ...
  if (requestData.organizationId !== organizationId || requestData.createdByUid !== decoded.uid) {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (freshRequest.createdByUid !== decoded.uid || freshRequest.organizationId !== organizationId) {
