  ...
  <body style="margin:0;background:#f5f6fa;font-family:Arial,sans-serif;color:#172033">
    <div style="max-width:600px;margin:0 auto;padding:28px 16px">
      <div style="background:#fff;border:1px solid #e7e9f0;border-radius:16px;padding:28px">
        <div style="font-size:14px;font-weight:700;color:#5b4ee8;margin-bottom:16px">ProjectMan</div>
        ...
        <a href="${safeUrl}" style="display:inline-block;background:#5b4ee8;color:#fff;text-decoration:none;border-radius:10px;padding:12px 18px;font-weight:700">Открыть ProjectMan</a>
      </div>
      <div style="font-size:12px;line-height:1.5;color:#7b8191;text-align:center;padding:14px 16px 0">
        Уведомление отправлено на подтверждённый адрес, используемый для входа в ProjectMan.
      </div>
    </div>
  </body>
</html>`;
  ...
  try {
    ...
    const response = await fetch(RESEND_EMAILS_URL, {
      ...
      body: JSON.stringify({
        from,
        to: [email],
        subject: `[ProjectMan] ${subjectTitle}`,
        text: `${subjectTitle}\n\n${text}\n\nОткрыть ProjectMan: ${appUrl()}`,
