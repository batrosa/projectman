// Email-уведомления для Google-пользователей и подтверждённых аккаунтов
// email/password. Отправка идёт через Resend Email API и всегда fail-open:
// проблема почтового провайдера не должна ломать создание/изменение задачи.
...
function appUrl() {
  ...
  return "https://projectman.online";
}

export function isEmailNotificationRecipient(user) {
  const email = cleanText(user?.email, 320).toLowerCase();
  const providerCanReceive = user?.authProvider === "google.com"
    || (user?.authProvider === "password" && user?.emailVerified === true);
  return providerCanReceive
    && email.length <= 320
    && EMAIL_RE.test(email);
}
  ...
  <body style="margin:0;background:#f5f6fa;font-family:Arial,sans-serif;color:#172033">
    <div style="max-width:600px;margin:0 auto;padding:28px 16px">
      <div style="background:#fff;border:1px solid #e7e9f0;border-radius:16px;padding:28px">
        <div style="font-size:14px;font-weight:700;color:#5b4ee8;margin-bottom:16px">ProjectSfera</div>
        ...
        <a href="${safeUrl}" style="display:inline-block;background:#5b4ee8;color:#fff;text-decoration:none;border-radius:10px;padding:12px 18px;font-weight:700">Открыть ProjectSfera</a>
      </div>
      <div style="font-size:12px;line-height:1.5;color:#7b8191;text-align:center;padding:14px 16px 0">
        Уведомление отправлено на подтверждённый адрес, используемый для входа в ProjectSfera.
      </div>
    </div>
  </body>
</html>`;
...
export async function sendEmailNotification(user, {
  ...
  if (!isEmailNotificationRecipient(user)) return { sent: false, reason: "not-email-recipient" };
  ...
  try {
    ...
    const response = await fetch(RESEND_EMAILS_URL, {
      ...
      body: JSON.stringify({
        from,
        to: [email],
        subject: `[ProjectSfera] ${subjectTitle}`,
        text: `${subjectTitle}\n\n${text}\n\nОткрыть ProjectSfera: ${appUrl()}`,
