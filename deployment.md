...
2. Зарегистрируйтесь или войдите с email и паролем (роль при регистрации всегда `reader`)
3. Закройте браузер и откройте снова
4. ✅ Вы должны автоматически войти в приложение

## Важные замечания

- **Роль назначается один раз при регистрации** (`reader`) и хранится в Firestore; для повышения до `admin` измените поле `role` документа пользователя вручную
