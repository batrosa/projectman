...
- **Канбан**: колонки «Назначенные / В работе / На проверке / Готово» с
...
- **Оформление**: системная / светлая / тёмная тема (адаптивная палитра —
  бренд-цвета web-версии в обеих темах), переключается в «Профиле».
- **Команда** (owner/admin): участники организации, смена ролей и исключение —
  через те же серверные действия, что веб-админка (`api/org updateMemberRole`
  / `removeMember`, ограничения сервер проверяет сам).
...
## Единый вход Google / Apple (web + iOS)

Код входа и явной привязки провайдеров реализован. Идентификатор аккаунта —
Firebase UID: после привязки Google или Apple к существующему аккаунту один и
тот же `users/{uid}` открывается в веб-версии и на iPhone. Совпавший email сам
по себе аккаунты не объединяет — это намеренная защита от неверного слияния.

Внешняя конфигурация выполнена 11.07.2026:

- Google и Apple включены в Firebase Authentication;
- `projectmanteko.vercel.app` добавлен в Firebase Authorized domains;
- App ID `com.holdingman.ios` поддерживает Sign in with Apple;
- Services ID `com.holdingman.web` связан с основным App ID;
- для веба зарегистрированы `projectmanteko.vercel.app` и callback
  `https://projectman-96d3c.firebaseapp.com/__/auth/handler`;
- новый `GoogleService-Info.plist` содержит `CLIENT_ID` и
  `REVERSED_CLIENT_ID`, а URL scheme хранится в `project.yml`/`Info.plist`;
- Apple OAuth key добавлен в Firebase. Его `.p8` не хранится в Git — резервную
  копию необходимо держать в защищённом хранилище, поскольку повторно скачать
  этот ключ у Apple нельзя.

Существующим пользователям Telegram: сначала войти через Telegram, затем в
«Профиль → Способы входа» подключить Google/Apple. Если сразу нажать новый
способ входа, Firebase может создать отдельный UID; приложение не выполняет
опасное автоматическое объединение по email.
...
## Чего НЕТ в этой версии (осознанно)

- **Ганта** — по решению владельца дорожная карта остаётся в веб-версии;
  в приложении только канбан.
- **Deep link из push в конкретную задачу** — данные (`taskId`, `projectId`)
  в payload уже есть, обработчик тапа — кандидат на следующую версию.
- **Рейтинга, календаря, файлов проекта** — веб-версия.

## Демо-режим (для отладки визуала)

DEBUG-сборка умеет запускаться с фиктивными данными без входа в аккаунт:

```bash
xcrun simctl launch <SIM> com.holdingman.ios --demo                    # табы
xcrun simctl launch <SIM> com.holdingman.ios --demo --demo-screen board # экран
# --demo-screen: board | task | team | mytasks | chat | notifications | settings
```
