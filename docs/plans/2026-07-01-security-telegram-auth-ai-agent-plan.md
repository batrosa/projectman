...
const SYSTEM_PROMPT_RULES = [
  "Ты — ИИ-помощник постановщика проекта внутри системы управления задачами.",
...
- ~~**`api/notify-telegram.js` (Task 2) is an open relay**: no auth/authorization check — any client can POST an arbitrary `chatId`+`text` and it will relay through the bot token.~~ **Resolved**: endpoint now requires a Firebase bearer token, loads the caller's user doc, resolves the target by `telegramChatId`, and only sends when the target user belongs to the caller's organization.
...
- ~~**`api/webhook.js` (Task 4) has no guard for missing `TELEGRAM_BOT_TOKEN`/`FIREBASE_WEB_API_KEY`**: if either env var is unset at deploy time, the webhook silently returns 200 OK to Telegram forever while never actually sending a reply or completing the Firestore update.~~ **Resolved for the current webhook path**: `api/webhook.js` returns `503` when `TELEGRAM_BOT_TOKEN` is missing, and the old Firebase Web API key path is no longer used by the bot-login flow.
...
- ~~**`isOrgOwnerOrAdmin(orgId)` (Task 5) doesn't actually scope by `orgId`**: any user whose own `orgRole` is `owner`/`admin` could update/delete another organization document.~~ **Resolved**: rules use `hasOrgRole(orgId, roles)`, which compares the caller's `users/{uid}.organizationId` to the target `orgId`; organization delete is now client-denied and performed server-side by `api/org`.
...
- ~~**`api/webhook.js`'s `/start` reply (Task 12) references a deleted UI path**: it tells users "Откройте ProjectSfera → Настройки → Telegram" for a linking code.~~ **Resolved**: `/start` now tells users to use "Войти через Telegram" / "Войти через бота" on the ProjectSfera login screen.
- **Post-deploy incident: `firebase-admin@14.x` crashed every request to `/api/agent-chat` with `ERR_REQUIRE_ESM`** (found and fixed within ~10 minutes of production deploy, commit `457f402`). Root cause: `firebase-admin@14.0.0` bumped its `jwks-rsa` dependency from `^3.1.0` to `^4.0.1`, and `jwks-rsa@4.x` requires `jose@^6.1.3` (pure ESM, no CJS build) while doing a top-level `require('jose')` in its own source — an unannounced breaking change in jwks-rsa's own package, invisible to our test suite since tests mock `adminAuth()` and never exercise the real Auth module's dependency chain end-to-end. Fixed by pinning `firebase-admin` to an exact `13.5.0` (last version depending on `jwks-rsa@^3.1.0` → `jose@^4.15.4`, dual CJS/ESM). Verified locally by directly requiring `firebase-admin/auth` and `jwks-rsa/src/utils.js` before redeploying, and confirmed live via curl (`401 Unauthorized` instead of `500`). **Lesson for future dependency bumps**: this class of bug (a mocked-auth test suite can't catch a real Admin SDK dependency crash) means any `firebase-admin` version bump should include a real, unmocked smoke test against a live deploy before being trusted — track re-attempting the upgrade to 14.x once jwks-rsa ships a fix.
