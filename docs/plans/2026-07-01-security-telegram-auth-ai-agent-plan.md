...
- ~~**`api/webhook.js`'s `/start` reply (Task 12) references a deleted UI path**: it tells users "Откройте ProjectMan → Настройки → Telegram" for a linking code.~~ **Resolved**: `/start` now tells users to use "Войти через Telegram" / "Войти через бота" on the ProjectMan login screen.
