# ProjectSfera Operations Runbook
...
2. Проверить заголовки ответа: `403` с `x-vercel-mitigated: challenge` означает edge protection, а не падение ProjectSfera.
...
Клиентский доступ к `auditLogs` запрещён Firestore rules. Просмотр выполняется через Firebase Console/GCP tools владельцем инфраструктуры. Если понадобится UI для аудита внутри ProjectSfera, его нужно делать отдельным admin-only API, не прямым client read.
