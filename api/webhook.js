            ...
            if (botSessionMatch) {
                ...
                if (unlinkResult.ok) {
                    replyText = unlinkResult.alreadyUnlinked
                        ? 'ℹ️ Этот Telegram уже не привязан к аккаунту ProjectMan.'
                        : '✅ Telegram отвязан.\n\nВойдите в нужный аккаунт ProjectMan через email, Google или Apple и подключите Telegram в личном кабинете.';
                } else if (unlinkResult.reason === 'last_provider') {
                    replyText = '❌ Нельзя отвязать единственный способ входа. Сначала подключите email, Google или Apple в ProjectMan.';
                } else {
                    replyText = '❌ Не удалось отвязать Telegram. Попробуйте позже или выполните отвязку в личном кабинете ProjectMan.';
                }
            } else if (text === '/START') {
