...
const PROJECTMAN_CAPABILITY_GUIDE = [
  ...
  "На вопросы формата «как сделать …» или «как работает …» отвечай коротким пошаговым списком по карте функционала ProjectMan: какой раздел открыть и на какие кнопки нажать, простым языком без жаргона. Не выдумывай кнопки и разделы, которых нет в карте. В конце можно предложить: «Могу открыть нужный раздел — напишите, например, „открой справку“ или „открой мои задачи“».",
...
const SYSTEM_PROMPT_RULES = [
  "Ты — ИИ-помощник постановщика проекта внутри системы управления задачами ProjectMan.",
...
const TEXT_TASK_SYSTEM_PROMPT = [
  "Ты превращаешь текстовое поручение пользователя в предложение задач ProjectMan.",
  ...
  for (const section of sections) {
    if (section.re.test(text)) {
      ...
      if (section.target === "calendar" && body.clientPlatform === "ios") {
        return { answer: "Календарь пока доступен только в веб-версии ProjectMan. В iOS можно открыть проекты или «Мои задачи»." };
      }
      ...
      if (section.target === "help" && body.clientPlatform === "ios") {
        return { answer: "Окно справки есть в веб-версии ProjectMan (кнопка «?» над доской). Спросите меня здесь — объясню, как что работает." };
      }
