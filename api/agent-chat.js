...
const PROJECTMAN_CAPABILITY_GUIDE = [
  ...
  "Гант (дорожная карта): вид «Гант» показывает задачи проекта полосами на временной шкале — от даты создания задачи до её дедлайна. Цвет полосы совпадает со статусом на доске: фиолетовый — назначена, оранжевый — в работе, жёлтый — на проверке, зелёный — готово; просроченные с красной обводкой; вертикальная линия отмечает сегодняшний день. Сверху выбирается год (стрелки и список) и период: «Весь год» — шкала по месяцам, либо конкретный месяц — шкала по дням с подсветкой выходных. Задачи БЕЗ дедлайна на Ганте не отображаются (счётчик «Без срока: N» показан в легенде) — чтобы задача попала на дорожную карту, ей нужно задать срок. Клик по полосе или названию открывает карточку задачи. Отдельной плановой даты начала у задачи нет — начало полосы это дата создания задачи.",
