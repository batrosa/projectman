...
const OFF_TOPIC_RESPONSE =
  "Я отвечаю только по ProjectSfera: проектам, задачам, срокам, исполнителям, файлам, уведомлениям и работе внутри вашей организации. По этому вопросу вне системы ответить не могу.";
...
const PROJECTMAN_CAPABILITY_GUIDE = [
  "Карта реального функционала ProjectSfera важнее общих знаний о системах управления проектами.",
  ...
  "Роли: владелец и админ управляют всеми задачами. Исполнитель может создавать задачи в доступных проектах (любые ответственные и доп. постановщики) и полностью управляет СОЗДАННЫМИ ИМ задачами (редактировать, удалять, принимать, возвращать на доработку); чужие задачи он только исполняет. Роль «модератор» временно скрыта и не назначается.",
  ...
  "Справка: заметная кнопка «?» рядом с «Новая задача» над доской открывает окно «Помощь» с описанием всех разделов. Пользователь может написать «открой справку», и она откроется.",
  "Чат с ИИ-агентом доступен только ролям от модератора и выше (owner/admin/moderator). У исполнителей кнопка «ИИ-агент» видна, но заблокирована, и сервер отклоняет их запросы — так организация экономит лимиты ИИ. Лента «Уведомления» доступна всем ролям.",
  "На вопросы формата «как сделать …» или «как работает …» отвечай коротким пошаговым списком по карте функционала ProjectSfera: какой раздел открыть и на какие кнопки нажать, простым языком без жаргона. Не выдумывай кнопки и разделы, которых нет в карте. В конце можно предложить: «Могу открыть нужный раздел — напишите, например, „открой справку“ или „открой мои задачи“».",
  ...
  "Уведомления: есть in-app «Уведомления агента» с прочтением/удалением уведомлений. Пользователи Google и пользователи с подтверждённой почтой email/password дополнительно получают уведомления на email. Telegram-уведомления работают при подключенном Telegram. Мобильные push-уведомления на iPhone работают через FCM/APNs, если пользователь вошёл в iOS-приложение, разрешил уведомления и устройство сохранило FCM-токен. События: новые задачи, задача на проверке, возврат на доработку, задача принята, напоминания/просрочки от server-side monitor. Нет подписок и пользовательских правил уведомлений.",
  ...
  "Если пользователь просит функцию, которой нет, прямо скажи «в ProjectSfera такой функции нет» и предложи ближайший реальный способ внутри текущего приложения.",
].join(" ");

const SYSTEM_PROMPT_RULES = [
  "Ты — ИИ-помощник постановщика проекта внутри системы управления задачами ProjectSfera.",
  ...
  "Отвечай по темам ProjectSfera: проекты, задачи, сроки, исполнители, файлы, уведомления, роли, вход и работа внутри организации.",
...
const TEXT_TASK_SYSTEM_PROMPT = [
  "Ты превращаешь текстовое поручение пользователя в предложение задач ProjectSfera.",
  ...
  "Ответственного сопоставляй только с участниками ProjectSfera из списка. Если форма имени в запросе склонена, верни точное имя из списка. Если участник не найден или есть сомнение, верни имя как написал пользователь.",
  ...
  "Если пользователь исправляет исполнителя фразой вроде «давай другому участнику», замени исполнителя, но не меняй название исходной задачи.",
...
export default async function handler(request, response) {
  ...
  // ИИ-агент (и траты OpenRouter-кредитов) — только для owner/admin/moderator.
  // Кнопка у исполнителя не кликабельна, но клиент ничего не доказывает:
  // единственная настоящая граница — эта серверная проверка. Управление
  // лентой уведомлений (delete_notification выше) исполнителям доступно —
  // оно кредиты не тратит.
  if (!["owner", "admin", "moderator"].includes(callerData?.orgRole)) {
    return response.status(403).json({ error: "ИИ-агент доступен ролям от модератора и выше. Обратитесь к владельцу или администратору организации." });
  }
...
export function resolveAgentNavigation({ message, body = {}, context = {}, callerData = {} }) {
  ...
  const sections = [
    ...
    { re: /(справк|помощ|инструкци|как\s+пользоваться)/u, target: "help", message: "Открываю справку." },
  ];
  for (const section of sections) {
    if (section.re.test(text)) {
      ...
      if (section.target === "calendar" && body.clientPlatform === "ios") {
        return { answer: "Календарь пока доступен только в веб-версии ProjectSfera. В iOS можно открыть проекты или «Мои задачи»." };
      }
      // Календарь — вид рабочей области ПРОЕКТА (переключатель Канбан/Гант/
      // Календарь под активным проектом). Без открытого проекта открывать
      // нечего — честно объясняем, где он находится.
      if (section.target === "calendar" && !String(body.projectId || "").trim()) {
        return { answer: "Календарь — это вид рабочей области проекта: переключатель «Канбан / Гант / Календарь» под активным проектом в списке слева. Сначала откройте проект (например, напишите «открой проект <название>»), и я открою его календарь." };
      }
      if (section.target === "help" && body.clientPlatform === "ios") {
        return { answer: "Окно справки есть в веб-версии ProjectSfera (кнопка «?» над доской). Спросите меня здесь — объясню, как что работает." };
      }
...
function buildTextTaskProposalFromRaw({ rawAnswer, users, projects, message = "", today = todayIsoDate() }) {
  ...
  if (Array.isArray(proposal?.tasks) && proposal.tasks.length === 0) {
    return {
      answer: "Не понял однозначно, кому поставить задачу. Назовите точные имена участников вашей организации и сформулируйте поручение одной фразой — я подготовлю карточку.",
      emptyTasks: true,
    };
  }
